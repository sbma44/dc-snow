...
var mapbox = new MapboxClient(process.env.MapboxAccessToken);
    ...
    if (fs.existsSync(filepath)) {
        ...
        while(offset < feature.geometry.coordinates.length) {
            ...
            if (f.geometry.coordinates.length > 1) {
                q.defer(function(cb) {
                    try {
                        mapbox.matching(f, { profile: 'mapbox.driving', geometry: 'geojson', gps_precision: 15 }, function(err, result) {
        ...
        q.awaitAll(function(err, results) {
            ...
            var output = merge(results.filter(function(x) { return ((x !== null) && (x !== undefined)); }));
